...
*/				
			...
			if (player_responce_jsonData.empty() && stream_map_len < 0 && hlsmpd_len <= 0)
			{
			...
			else
			{
				...
				if (!player_responce_jsonData.empty() && Reader.parse(player_responce_jsonData, Root) && Root.isObject())
				{
							...
							if (formats.isArray())
							{
								for(int j = 0, len = formats.size(); j < len; j++)
								{
									...
									if (format.isObject())
									{
										...
										if (mimeType.isString()) item.format = GetCodecName(HostUrlDecode(quality.asString()));
										if (fps.isDouble())
										{
										...
										else if (cipher.isString())
										{
											string u, signature, sigName = "signature";
											array<string> params = cipher.asString().split("&");
			...
			if (!final_url.empty())
			{
				...
				{
					string api = "https://www.googleapis.com/youtube/v3/videos?id=" + videoId + "&part=snippet,statistics&fields=items/snippet/title,items/snippet/publishedAt,items/snippet/channelTitle,items/snippet/description,items/statistics";
